...
public class SchoolHandle implements BaseOperateInterface {
	...
	* @return void
	...
	public void save(List<StandardDataRelation> relationList) throws Exception{
		...
		for(int i=0;i<relationList.size();i++){
			...
			if("ID".equalsIgnoreCase(standardName)){
				...
				if(df!=null&&df.getValue()!=null){
					...
					String findStr=" select count(*) from reps_sch_school where id='"+idValue+"' ";
					int count=jdbcDao.queryForInt(findStr);
					if(count>0){
						throw new Exception("添加--》数据对象【学校基本信息】的数据项【"+standardName+"="+idValue+"】对应学校信息已存在");
					}
				}
				
			}
			//判断对应机构信息是否存在
			if("机构基本信息".equalsIgnoreCase(standardName)){
				//判断数据库中是否存在该记录
				if(df!=null&&df.getValue()!=null){
					String orgIdValue=df.getValue().toString();
					String findStr=" select count(*) from reps_sys_organize where id='"+orgIdValue+"' ";
					int count=jdbcDao.queryForInt(findStr);
					if(count<1){
						throw new Exception("添加--》数据对象【学校基本信息】的数据项【"+standardName+"="+orgIdValue+"】对应机构信息不存在");
					}
				}
			}
		...
		if(insertFields.length()>0&&insertValues.length()>0){
			String insert = "Insert into reps_sch_school (" + insertFields.toString() + ")values(" + insertValues.toString() + ")";
			jdbcDao.update(insert, values.toArray());
		}else{
			throw new Exception("添加--》数据对象【学校基本信息】的数据项不能为空");
		}
	}
	...
	* @return boolean
	...
	public boolean update(List<StandardDataRelation> relationList) throws Exception {
		...
		StringBuffer update = new StringBuffer("update reps_sch_school set ");
		List<Object> values = new ArrayList<Object>();
		String idValue=null;
		...
		for(int i=0;i<relationList.size();i++){
			...
			if("ID".equalsIgnoreCase(standardName)){
				...
				if(df!=null&&df.getValue()!=null){
					...
					int count=jdbcDao.queryForInt(findStr);
					if(count<1){
						throw new Exception("更新--》数据对象【学校基本信息】的数据项【"+standardName+"="+idValue+"】对应学校信息不存在");
					}
				}
			}
			//判断对应机构信息是否存在
			if("机构基本信息".equalsIgnoreCase(standardName)){
				...
				if(df!=null&&df.getValue()!=null){
					...
					int count=jdbcDao.queryForInt(findStr);
					if(count<1){
						throw new Exception("更新--》数据对象【学校基本信息】的数据项【"+standardName+"="+orgIdValue+"】对应机构信息不存在");
					}
				}
			}
			if(df!=null){
		...
		if(StringUtils.isNotBlank(idValue)){
			if(values.size()>=1){
				...
				throw new Exception("更新--》数据对象【学校基本信息】中没有填写要更新的数据项");
			}
		}else{
			throw new Exception("更新--》数据对象【学校基本信息】数据项【ID】不能为空");
		}
		return true;
	}
	...
	* @return boolean
	...
	public boolean delete(String id) throws Exception{
		...
		if (StringUtils.isBlank(id)) {
		    throw new Exception("删除记录错误:未设置数据对象【学校基本信息】的ID");
		}
		String findStr=" select count(*) from reps_sch_school where id='"+id+"' ";
		//检查id对应的记录是否存在
		int count=jdbcDao.queryForInt(findStr);
		if(count==1){
			jdbcDao.execute("delete from reps_sch_school where id='"+id+"' ");
			flag=true;
		}else{
			throw new Exception("删除--》数据对象【学校基本信息】的数据项【ID="+id+"】对应学校信息不存在");
		}
		return flag;
	}
