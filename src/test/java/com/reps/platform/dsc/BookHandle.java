...
public class BookHandle implements BaseOperateInterface{
	...
	* @return void
	...
	public void save(List<StandardDataRelation> relationList) throws Exception{
		...
		for(int i=0;i<relationList.size();i++){
			...
			if("ID".equalsIgnoreCase(standardName)){
				...
				if(df!=null&&df.getValue()!=null){
					...
					String findStr=" select count(*) from reps_res_book where id='"+idValue+"' ";
					int count=jdbcDao.queryForInt(findStr);
					if(count>0){
						throw new Exception("添加--》数据对象【教材基本信息】已存在数据项【"+standardName+"="+idValue+"】对应的教材信息");
					}
				}
			}
		...
		if(insertFields.length()>0&&insertValues.length()>0){
			String insert = "Insert into reps_res_book (" + insertFields.toString() + ")values(" + insertValues.toString() + ")";
			jdbcDao.update(insert, values.toArray());
		}else{
			throw new Exception("添加--》数据对象【教材基本信息】的数据项不能为空");
		}
	}
	...
	* @return boolean
	...
	public boolean update(List<StandardDataRelation> relationList) throws Exception {
		...
		StringBuffer update = new StringBuffer("update reps_res_book set ");
		List<Object> values = new ArrayList<Object>();
		//记录ID值
		String idValue=null;
		...
		for(int i=0;i<relationList.size();i++){
			...
			if("ID".equalsIgnoreCase(standardName)){
				...
				if(df!=null&&df.getValue()!=null){
					...
					int count=jdbcDao.queryForInt(findStr);
					if(count<1){
						throw new Exception("更新--》数据对象【教材基本信息】不存在数据项【"+standardName+"="+idValue+"】对应的教材信息");
					}
				}
			}
		...
		if(StringUtils.isNotBlank(idValue)){
			if(values.size()>=1){
				...
				throw new Exception("更新--》数据对象【教材基本信息】中没有填写要更新的数据项");
			}
		}else{
			throw new Exception("更新--》数据对象【教材基本信息】数据项【ID】不能为空");
		}
		return true;
	}
	...
	* @return boolean
	...
	public boolean delete(String id) throws Exception{
		...
		if (StringUtils.isBlank(id)) {
		    throw new Exception("删除记录错误:未设置数据对象【教材基本信息】的ID");
		}
		String selectStr=" select count(*) from reps_res_book where id='"+id+"' ";
		int count=jdbcDao.queryForInt(selectStr);
		//检查id对应的记录是否存在
		if(count==1){
			jdbcDao.execute("delete from reps_res_book where id='"+id+"' ");
			flag=true;
		}else{
			throw new Exception("删除--》数据对象【教材基本信息】不存在数据项【ID="+id+"】对应的教材信息");
		}
		return flag;
	}
