...
<#if !multiple>
		...
		<#else>
			<#assign href = href + "${contextPath!}${actionBasePath!}/sys/chooseorganize/singletree.mvc?showName=${name!}&hideName=${hideName!}&dialogId=${hideId!}dialog&showRoot=${showRoot?string}&organize.type=${dataType!}&callBack=${callBack!}" />
			</#if>
	<#else>
		<a <#if cssClass??>class="${cssClass!}"</#if> target="dialog" style="<#if style??>${style}</#if>" href="#" onClick="${hideId!}organizeClick();" rel="${hideId!}-organize-rel" title="组织机构" data-options='{"id":"${hideId!}dialog",width:280,height:300,"drawable":${drawable?string("true","false")},"inIframe":true}'><#if value??>${value!}<#else>组织机构选择</#if></a>
		<#if url??>
			<#assign href = href + "${url!}hideName=${hideName!}&multiple=${multiple?string}&dialogId=${hideId!}dialog&showRoot=${showRoot?string}&organize.type=${dataType!}&callBack=${callBack!}&multipleCascade=${multipleCascade?string('true','false')}" />
		<#else>
			<#assign href = href + "${contextPath!}${actionBasePath!}/sys/chooseorganize/multipletree.mvc?hideName=${hideName!}&multiple=${multiple?string}&dialogId=${hideId!}dialog&showRoot=${showRoot?string}&organize.type=${dataType!}&callBack=${callBack!}&multipleCascade=${multipleCascade?string('true','false')}" />
		</#if>
 </#if>
