...
<reps:container layout="true">
	<reps:panel id="mytop" dock="top" action="${ctx}/reps/school/executeimport/teacherlist.mvc" method="post" formId="queryForm">
		...
		<reps:footbar>
			...
			<!-- <reps:button value="返回" cssClass="return-a" action="${ctx}/reps/school/teacher/zxxlist.mvc"/> -->
		</reps:footbar>
	</reps:panel>
...
<script type="text/javascript">
 		...
 		if(data.success) {
 			...
 			if (data.result == false) {
 				messager.confirm("导入过程中存在错误数据,是否查询错误数据", {
 					okCall: function(){
 						window.location.href = "${ctx}/reps/school/executeimport/teacherlist.mvc?orgTypes=" + $("#orgTypes").val() + "&status=" + 2 + "&isManager=" + $("#isManager").val();
 					},
					cancelCall:function(){
						refresh();
					}
	...
	function refresh() {
		...
		if (isManager == '1') {
			window.location.href = "${ctx}/reps/school/executeimport/teacherlist.mvc?orgTypes=" + $("#orgTypes").val() + "&isManager=" + isManager;
		} else {
