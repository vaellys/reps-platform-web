...
<body>
	<reps:container layout="true">
		<reps:panel id="top" dock="top" method="post" action="${ctx}${actionBasePath}/sys/chooseuser/multiple.mvc" formId="queryForm">
			<reps:formcontent parentLayout="true" style="width:80%;">
				...
				<input name="allowBlank" type="hidden" value="${allowBlank}" />
		...
		<reps:panel id="mybody" dock="center" border="true">
			<reps:grid id="mygrid" items="${list}" form="queryForm" var="user" pagination="${pager}">
				<reps:gridrow>
					...
					<reps:gridfield title="姓名" width="15">${user.account.person.name}</reps:gridfield>
					<reps:gridfield title="登录名" width="15">${user.account.loginName}</reps:gridfield>
					...
					<reps:gridfield title="选择" width="10" align="center">
						<c:choose>
							<c:when test="${fn:contains(hideNameValue,user.id)}">
								<input type="checkbox" checked="checked" name="user" value="${user.id}" nameAttr="${user.account.loginName}" data-options="{'id':'${user.id}','loginName':'${user.account.loginName}','name':'${user.account.person.name}'}" />
							</c:when>
							<c:otherwise>
								<input type="checkbox" name="user" value="${user.id}" nameAttr="${user.account.loginName}" data-options="{'id':'${user.id}','loginName':'${user.account.loginName}','name':'${user.account.person.name}'}" />
							</c:otherwise>
						</c:choose>
					</reps:gridfield>
				</reps:gridrow>
			...
			for(var i in dataJson){
				...
				if(0 != i){
					if((dataJson.length - 1) == i){
						$("#selecteUser").append("<span id='show"+dataObj["id"]+"'nameAttr='"+dataObj["loginName"]+"'idAttr='"+dataObj["id"]+"' >"+dataObj["loginName"]+"</span>");
					}else{
						$("#selecteUser").append("<span id='show"+dataObj["id"]+"'nameAttr='"+dataObj["loginName"]+"'idAttr='"+dataObj["id"]+"' >"+dataObj["loginName"]+", </span>");
					}
