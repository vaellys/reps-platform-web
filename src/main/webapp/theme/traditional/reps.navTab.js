...
var navTab = {
		...
		this._getTabs().each(function(iTabIndex){
			$(this).unbind("click").click(function(event){
				...
				if("main" ==  tabId){
					$.ajax({
						type:"post",
						url:REPS.getProjectName() + "/" +"ajaxdesk.mvc",
						async:false,
						dataType:"html",
	...
	_closeTab: function(index, openTabid){
		...
		this._getPanels().eq(index).trigger(REPS.eventType.pageClear).remove();
	...
	closeAllTab: function(){
		...
		this._getPanels().filter(":gt(0)").trigger(REPS.eventType.pageClear).remove();
	...
	_closeOtherTab: function(index){
		...
		if (index > 0) {
			...
			this._getPanels().not(str$).filter(":gt(0)").trigger(REPS.eventType.pageClear).remove();
	...
	checkTimeout:function(){
		var json = REPS.jsonEval(this.getCurrentPanel().html());
		if (json && json.statusCode == REPS.statusCode.timeout) this.closeCurrentTab();
	},
	openExternal:function(url, $panel, op){
		var ih = navTab._panelBox.height();
		var externalFrag;
		if(op && op.postmethod){
			var inputs = "";
			for(var key in op.data){
				 inputs += '<input type="hidden" name="'+key+'" value="'+op.data[key]+'" />';
			}
			externalFrag = '<div style="display:none;"><form id="postForm'+op.tabid+'" action="'+url+'" target="hiddenIframe'+op.tabid+'" type="post" name="" method="post">'+inputs+'</form></div><iframe name="hiddenIframe'+op.tabid+'" style="width:100%;height:{height};display:none;" frameborder="no" border="0" marginwidth="0" marginheight="0"></iframe>';
			$panel.html(externalFrag.replaceAll("{height}", ih+"px"));
			$panel.find("#postForm"+op.tabid).submit();
			$panel.find("iframe[name='hiddenIframe"+op.tabid+"']").show();
		}else{
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		var op = $.extend({title:"New Tab", data:{}, fresh:true, external:false,postmethod:false}, options);
		...
		if (iOpenIndex >= 0){
			...
			if(op.fresh || $tab.attr("url") != url) {
				...
				if (op.external || url.isExternalUrl()) {
					...
					navTab.openExternal(url, $panel ,op);
				} else {
