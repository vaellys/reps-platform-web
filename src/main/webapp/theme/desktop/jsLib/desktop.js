...
myLib.desktop.win={
	winHtml:function(title,url,id){
		...
		+"' id='myFrame_"
		+id
		+"' class='winframe' scrolling='auto' width='100%' src='"+url+"'></iframe></div>";
		},
    ...
    newWin:function(options){
		...
		if($(".windows").size() == 0){
			...
			wTop=myLib._is(options['WindowPositionTop'],"Number")?options['WindowPositionTop']+dxy/2:(wh['h']-options['WindowHeight'])/2+dxy/2 - 50;
		}
		var wLeft=myLib._is(options['WindowPositionLeft'],"Number")?options['WindowPositionLeft']+dxy-panelLeft.left:(wh['w']-options['WindowWidth'])/2+dxy-panelLeft.left - 100;
						...
						_this.minimize($newWin);		   
						 ...
						 if(wins.is(":hidden")){
						      ...
						      myLib.desktop.win.switchZindex(wins);
								 ...
								 var winTabId = id.substring(0, 5) + "Tab" + id.substring(5);
								 var $winTab = $("#"+winTabId);
								 $winTab.click();
								 }
 						 }
